      ...
      <div className="spa-services__grid">
        {services.map((service, index) => (
          <div key={index} className="spa-services__card">
            <img src={service.image} alt={service.title} className="spa-services__image" />
            <h3 className="spa-services__card-title">{service.title}</h3>
            {service.price && <p className="spa-services__price">Giá từ {service.price}</p>}
            <a href={service.link} className="spa-services__link">Tìm hiểu thêm &gt;</a>
          </div>
        ))}
      </div>
    </div>
