      ...
      <div className="hair-services__grid">
        {services.map((service, index) => (
          <div key={index} className="hair-services__card">
            <img src={service.image} alt={service.title} className="hair-services__image" />
            <h3 className="hair-services__card-title">{service.title}</h3>
            {service.price && <p className="hair-services__price">Giá từ {service.price}</p>}
            <a href={service.link} className="hair-services__link">Tìm hiểu thêm &gt;</a>
          </div>
        ))}
      </div>
    </div>
